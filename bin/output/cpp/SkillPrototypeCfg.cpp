/***
 * SkillPrototypeTbl.cpp
 * SkillPrototypeCfg.cpp
 *
 * @Author: abaojin
 */
namespace Config
{
	public struct SkillPrototypeTbl
    {
		// #id
		public ﻿int Id;
		// 职业说明代码不用
		...
		public string Shape;
		// 形状参数1(扇形半径米矩形宽米)
		public string ShapeArgument1;
		// 形状参数2(扇形弧度矩形长米)
		public string ShapeArgument2;
		// 技能释放距离,空放时使用该距离的点作为技能释放点, 否则使用被选取目标坐标为释放点
		public string Distance;
		// 击退攻击次数
		public string RepelHits;
		// 击退距离
		public string RepelDistance;
		// 位移攻击次数
		public string MoveHits;
		// 位移距离
		public string MoveDistance;
		// 每一段打击的伤害分配比例(万分之), 总和应该为10000, 分摊的是AtkArgument字段带来的伤害
		public string HitDamagePercent;
		// 每一段打击的护甲值伤害百分比
		public string ArmorDamagePercent;
		// 是否解除控制效果(1是0否)
		public int ReliveRestrict;
		// 技能释放时间(持续时间LastTime)内免疫控制1是0否
		public int LastImmuneRestrict;
		// 起始动作id
		...
		public int AtkArgument;
		// 对护甲值伤害
		public int ArmorArgument;
		// 即时攻击百分比系数(万分之)升级加值
		public int AtkArgumentLevelup;
		// 即时攻击护甲值百分比系数(万分之)升级加值
		public int ArmorArgumentLevelup;
		// 施加状态持续时间(秒)
		public int StateLastTime;
		// 持续时间内魔法罩吸收伤害比例
		public int StateShieldResist;
		// 持续时间内魔法罩吸收伤害上限
		public int StateShieldHp;
		// 施加状态时间内眩晕
		public int StateStun;
		// 施加状态时间内定身
		public int StateRestrain;
		// 施加状态时间内免控
		public int StateImmuneRestrict;
		// 施加状态时间内无敌
		public int StateInvincible;
		// 施加状态时间内嘲讽
		public int StateSneer;
		// 施加状态时间内吸血比例(万分之)
		public int StateSuck;
		// 施加状态时间内生命上限百分比掉血(万分之)
		public int StatePercentHP;
		// 施加状态时间内总攻击系数万分之(对敌伤害对友治疗)
		public int StateAtkArgument;
		// 施加状态时间内总攻击百分比升级系数加值
		public int StateAtkArgumentLevelup;
		// 持续时间内影响角色的属性枚举列表
		public string StateRoleAttributeList;
		// 持续时间内影响角色的属性变化方式列表, 1百分比2绝对值, 与属性列表一一对应
		public string StateRoleAttributeChange;
		// 持续时间内影响角色的属性数值列表, 正+负-, 与属性列表一一对应
		public string StateRoleAttributeValue;
		// 持续效果特效
		public int LastEffectsID;
		// 区域基准点据玩家距离
		public float AreaDistance;
		// 区域效果持续时间(秒)
		public int AreaEffectTime;
		// 区域效果形状(1线2圆)
		public int AreaEffectShape;
		// 区域效果形状参数1(线:长度米; 圆形:半径米)
		public int AreaEffectShapeArgument;
		...
		// 持续状态时间内物理免疫
		public int StateImmunePhysics;
		// 持续状态事件内魔法免疫
		public int StateImmuneMagic;
